...
export default function Home() {
  ...
  // Already logged in with team → go to dashboard
  if (user && profile && team) {
    if (typeof window !== 'undefined') window.location.href = '/dashboard';
    return null;
  }

  // Logged in but no team → show team selection
  if (user && profile && !team) {
    return (
  ...
  if (mode === 'login' || mode === 'register') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-950 p-4">
        <div className="w-full max-w-sm">
          <button onClick={() => setMode('landing')} className="text-gray-500 text-sm mb-6 block">&larr; 돌아가기</button>
          <h2 className="text-xl font-black text-white mb-6">
            {mode === 'login' ? '로그인' : '회원가입'}
          </h2>

         <input value={name} onChange={e => { setName(e.target.value); setEmail(e.target.value.replace(/\s/g,'') + '@student.local'); }}
              placeholder="이름"
              className="w-full px-4 py-3 bg-gray-900 border border-gray-700 rounded-xl text-white text-sm mb-3 focus:border-cyan-400 transition" />
          ...
          }} className="w-full py-3 bg-cyan-500 text-white font-bold rounded-xl mt-2 transition hover:bg-cyan-600">
            {mode === 'login' ? '로그인' : '가입하기'}
          </button>
